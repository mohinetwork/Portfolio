...
export default function PortfolioStars() {
  ...
  useEffect(() => {
    const fetchStars = async () => {
      try {
        // Try our API first
        const response = await fetch('/api/github-stars?owner=lostingness&repo=sleek-portfolio')
        ...
        if (data.success && data.stars > 0) {
          ...
          const githubResponse = await fetch('https://api.github.com/repos/lostingness/sleek-portfolio')
  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <a
          href="https://github.com/lostingness/sleek-portfolio"
          target="_blank"
